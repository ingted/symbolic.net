...
type CD<'K, 'V> = ConcurrentDictionary<'K, 'V>
...
module Evaluate =
    ...
    let rec evaluate2 (
            ...
            , symbolValues:ConcurrentDictionary<string, FloatingPoint> //PassedIn
        ...
        | FunInvocation (Symbol parentFxName, paramValueExprList) ->
            ...
            if keyWord.ContainsKey parentFxName then
                ...
                let rec nestedFxHandler
                    ...
                    let reNestedFxHandle = nestedFxHandler symbolValues_ sysVarValueStack_
                        ...
                        | FunInvocation ((Symbol sb), origParamExp) when fd.ContainsKey sb ->
                            let evaluatedValue =
                                ...
                                |> List.map (fun param ->
                                    let newSymbolName = $"__{sb}_{Guid.NewGuid().ToString()}__"
                                    let newSymbol = Symbol newSymbolName
                                    let paramValue = reEvaluate2 symbolValues_ sysVarValueStack_ param
                                    symbolValues_.TryAdd(newSymbolName, paramValue) |> ignore
                                    Identifier newSymbol
                                )
                            ...
                            let evaluatedFunValue = reEvaluate2 symbolValues_ sysVarValueStack_ (FunInvocation ((Symbol sb), evaluatedValue))
                            symbolValues_.TryAdd(newSymbolNameAggRst, evaluatedFunValue) |> ignore
                            FAkka.Microsoft.FSharp.Core.LeveledPrintf.frintfn FAkka.Microsoft.FSharp.Core.LeveledPrintf.PRINTLEVEL.PWARN "Dynamic symbol added: %A" newSymbolAggRst
                            sl, Identifier newSymbolAggRst
                ...
                let r = 
                    ...
                    | DTExp (parentFxParamSymbols, parentFxBody) ->
                        ...
                        | FunInvocation _ ->                       
                            ...
                            reEvaluate2 symbolValues updatedStack parentFxBody
                            //evaluate2 (symbolValues, sysVarValueStack, (Some (fun () -> sysVarValueStack.TryPop() |> ignore))) parentFxBody
                        | _ ->
                            let uSL, frv = nestedFxHandler symbolValues updatedStack parentFxParamSymbols parentFxBody
                            let rFrv, rUSl = renameSymbols (uSL, frv) //20250413 symbol 名稱統一化後，快取才有意義
                        ...
                        let rec evalProc
                            ...
                            | (paramSymbols, defBody) :: restProcList ->
                                ...
                                let rst =
                                    match defBody with
                                    | DBFun almightFun ->
                                        let sv = almightFun gContext sContext prevOutputOpt sysVarValueStack paramValueExprListOpt (sysVarValueStack.IsNone)
                                ...
                                evalProc restProcList (Some rst) updatedStack          None                      (procStepId())

                        let finalOutput =
                                evalProc     procList  None      (Some sCtxFF)        (Some paramValueExprList)   (procStepId())
                        finalOutput
