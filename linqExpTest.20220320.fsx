...
let obj2ValueToInject =
                    <@ Func<obj, MathNet.Symbolics.Value>(fun j ->
                        match j with
                        | :? float as fv ->
                            MathNet.Symbolics.Value.Approximation (MathNet.Symbolics.Approximate.real fv)
                        | :? MathNet.Numerics.LinearAlgebra.Vector<float> as vfv ->
                            MathNet.Symbolics.Value.RealVec vfv
                        | :? MathNet.Numerics.LinearAlgebra.Matrix<float> as mfv ->
                            MathNet.Symbolics.Value.RealMat mfv
                        )
                        @> |> Lambda.toExpression :> Expression :?> LambdaExpression

replaceName "j" "v" obj2ValueToInject //not works


let exprObj2ValueToInject = //: Expression<Func<obj, MathNet.Symbolics.Value>> =
    ExprHelper<Func<obj, MathNet.Symbolics.Value>>.Quote (fun j ->
                        match j with
                        | _ when j.GetType() = typeof<float> ->
                            //failwith "orz"
                            //MathNet.Symbolics.Value.fromDouble  0.0
                            MathNet.Symbolics.Value.Approximation (MathNet.Symbolics.Approximation.Real (j :?> float))
                        | :? MathNet.Numerics.LinearAlgebra.Vector<float> as vfv ->
                            MathNet.Symbolics.Value.RealVec vfv
                        | :? MathNet.Numerics.LinearAlgebra.Matrix<float> as mfv ->
                            MathNet.Symbolics.Value.RealMat mfv
...
exprObj2ValueToInject = obj2ValueToInject //false
replaceName "j" "v" exprObj2ValueToInject
